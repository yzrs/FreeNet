...
from outer_tools.lib.nms.nms import oks_nms
from outer_tools.lib.nms.nms import soft_oks_nms

from outer_tools.lib.dataset_animal.kpt_2d_base import Kpt2dSviewRgbImgTopDownDataset
from outer_tools.lib.dataset_animal.dataset_info import DatasetInfo
from outer_tools.lib.dataset_animal import ap_10k_animal_pose_tigdog_info
...
class AP10KAnimalTigDogPoseDataset(Kpt2dSviewRgbImgTopDownDataset):
    def __init__(self,
                 cfg,
                 root,
                 image_set,
        ...
        if image_set == 'train':
            ...
            ann_file = self.root + 'annotations/ap10k_animalpose_tigdog_train.json'
        elif image_set == 'val':
            logger.info('Loading validation annotations')
            ann_file = self.root + 'annotations/ap10k_animalpose_tigdog_val.json'
        else:
            # No test dataset!!!
            logger.error('No testing annotations for AP-10K + Animal Pose + tigdog Union Dataset')
            return
        ...
        self.id2name, self.name2id = self._get_mapping_id_name(
            self.coco.imgs)
        dataset_info = ap_10k_animal_pose_tigdog_info.dataset_info
        ...
        # loading anns from random 1/10 AP-10K and Animal Pose Dataset by imageId
        self.few_shot_setting = True
        if self.few_shot_setting and image_set == 'train':
            random_percentage = cfg.IMG_ANNOTATED
            with open('data/label_list/union_random_{}_img_ids.json'.format(random_percentage), 'r') as fp:
                self.imageid_annot = json.load(fp)
            logger.info('number of annotated images: {}'.format(len(self.imageid_annot)))
            with open('data/label_list/union_random_{}_img_ann_ids.json'.format(random_percentage),'r') as fp:
                self.imageid_annot_map = json.load(fp)
            self.img_ids = self.imageid_annot

        self.db, self.id2Cat = self._get_db()
        logger.info('=>{} dataset load {} samples'.format(image_set,len(self.db)))
    ...
    def _load_coco_keypoint_annotation_kernel(self, img_id):
        ...
        for obj in objs:
            ...
            if self.image_set == 'train':
                if obj['id'] not in self.imageid_annot_map[str(obj['image_id'])]:
                    continue
            x, y, w, h = obj['bbox']
            x1 = max(0, x)
            y1 = max(0, y)
    ...
    def evaluate(self, cfg, preds, output_dir, all_boxes, img_path,
        ...
        if platform.system() == 'Windows':
            prefix = '../dataset/ap10k_animalpose_tigdog/data\\'
        else:
            prefix = '../dataset/ap10k_animalpose_tigdog/data/'
        prefix_length = len(prefix)
        img_path = [s[prefix_length:] for s in img_path if s.startswith(prefix)]
