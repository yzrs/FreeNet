...
from train_utils.dataset import MixKeypoint,CocoKeypoint
from train_utils.ssl_utils import ours_ap10k_animalpose_tigdog
...
def main(cfg,args):
    ...
    train_dataset_info = [{"dataset":"ap_10k","mode":"train"},
                          {"dataset":"animal_pose","mode":"train"},
                          {"dataset":"tigdog","mode":"train"}]
    data_root = args.data_root

    train_label_dataset = MixKeypoint(root=data_root,merge_info=train_dataset_info,transform=data_transform['train'],num_joints=24)
    train_unlabel_dataset = MixKeypoint(root=data_root,merge_info=train_dataset_info,transform=data_transform['train'],num_joints=24)
    ...
    ours_ap10k_animalpose_tigdog(cfg,args,train_label_loader,train_unlabel_loader,t_model, s_model,t_optimizer,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--pretrained-weights-name', default='union_sl_0.1.pth',
                        type=str, help='pretrained weights name')
    parser.add_argument('--sample-ratio', default=0.1, type=float, help='dataset samplel ratio')
    ...
    parser.add_argument('--keypoints-path', default="./info/union_definition.json", type=str,
                        help='keypoints_format.json path')
    parser.add_argument('--fixed-size', default=[256, 256], nargs='+', type=int, help='input size')
    parser.add_argument('--num-joints', default=24, type=int, help='num_joints')
    # best info
    ...
    parser.add_argument('--cfg',default='outer_tools/experiments/ap10k/hrnet/w32_256x192_adam_lr1e-3_union.yaml',
    ...
    source_file_path = 'ours_ap10k_animalpose_tigdog.py'
    target_file_path = os.path.join(output_dir, 'ours_ap10k_animalpose_tigdog.py')
